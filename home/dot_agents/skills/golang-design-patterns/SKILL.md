...
Idiomatic Go patterns for production-ready code. For error handling details see the `golang-error-handling` skill.
...
Assert interface satisfaction at compile time with `var _ Interface = (*Type)(nil)` — catches a broken implementation immediately instead of at first use.
