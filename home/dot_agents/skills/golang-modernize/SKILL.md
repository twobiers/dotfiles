...
user-invocable: true
license: MIT
compatibility: Designed for Claude Code, opencode, or similar AI coding agents, and for projects using Golang.
metadata:
  author: samber
  version: "1.2.5"
---
...
- **Full-scan mode** (explicit `/golang-modernize` invocation or CI): dispatch up to 5 parallel sub-agents — one scans deprecated packages and API replacements, one scans language feature opportunities (range-over-int, min/max, any, iterators), one scans standard library upgrades (slices, maps, cmp, slog), one scans testing patterns (t.Context, b.Loop, synctest), one scans tooling and infra (golangci-lint v2, govulncheck, PGO, CI pipeline) — then consolidate and prioritize by the migration priority guide. The scan itself is read-only; once consolidated, apply the resulting codebase-wide rewrite on an isolated branch or worktree so a sweeping multi-file modernization never touches the developer's main tree until reviewed.
...
7. **For large codebases**, parallelize the scan across up to 5 sub-agents, each targeting a different modernization category (e.g. deprecated packages, language features, standard library upgrades, testing patterns, tooling and infra). Once scanning is done and changes are ready to apply, do so on an isolated branch or worktree — a codebase-wide modernization sweep touches many files at once, and isolation keeps the main tree safe to abandon or review before merging.
...
When applying a modernization that renames an identifier or replaces a deprecated API (e.g. `reflect.PtrTo` → `PointerTo`, `math/rand` → `math/rand/v2`), prefer `gopls`'s safe rename (via your agent's LSP integration or the `gopls rename` CLI) over a blind Edit or grep/sed sweep — it updates every call site and refuses a rename that would break interface satisfaction, and post-edit diagnostics catch compile errors across the rewritten files.
...
26. Set up AI-driven code review in CI to guide review per area

## Related Skills

See `golang-testing`, `golang-error-handling` skills.
