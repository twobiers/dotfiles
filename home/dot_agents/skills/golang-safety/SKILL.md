...
Maps MUST NOT be accessed concurrently — guard with a `sync.Mutex`/`sync.RWMutex` or use `sync.Map`.
...
A goroutine that blocks forever (on a channel nobody sends to, a context that never cancels) never gets garbage collected. Always give a goroutine a way out — a `context.Context` it selects on, or a done channel it can be signaled through.
