...
- **Simple-sweep mode** — a single mechanical, behavior-preserving transform applied tree-wide in one autonomous pass, with no step depending on another (e.g. one `gofmt -r`/`eg`/`modernize` fixer, verified green). Do not use this mode for a multi-step refactor needing progressive human review between merges — see the "Human-in-the-loop, not autonomous multi-stage" note in [workflow.md](references/workflow.md).
...
**Dependencies:** `gopls` (primary actuator) — `go install golang.org/x/tools/gopls@latest`. Optional: `golangci-lint`, `benchstat`, `deadcode`, `eg`, `gopatch`. Wire it up via your agent's LSP integration or MCP server config — every other reference to it in this skill assumes it's already installed and reachable.
...
- **Load the `golang-security` skill (and `golang-safety` for internal-correctness risk) whenever a step changes code logic, not just its shape.**
...
- [workflow.md](references/workflow.md) covers the full choreography — read it before planning any multi-step refactor:
  ...
  - why an unsupervised, autonomous multi-stage run is the wrong tool for this
